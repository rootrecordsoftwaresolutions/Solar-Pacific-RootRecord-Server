...
def unpack(obj):
    ...
    for key in ("samples","records","data","readings","telemetry"):
        if isinstance(obj.get(key),list): return unpack(obj[key])
    return []
...
def migrate(source,serial,model,alias,db=None):
    ...
    try:
        ...
        for path in files_for(Path(source)):
            ...
            for envelope,fields in unpack(obj):
                ...
                for key,value in fields.items():
                    ...
                    add_device_measurement(conn,observation_id=obs,metric_key=key,value=value,
                                           state="missing" if value is None else "measured")
                imported+=1
        conn.commit()
...
def main():
    ...
    args=p.parse_args()
    migrate(args.source,args.serial,args.model,args.alias,args.db)
    return 0
