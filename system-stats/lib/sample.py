...
def _read_mem():
    ...
    try:
        ...
        if total and avail is not None:
            return total, avail, 100.0 * (1.0 - avail / total), "measured"
    except Exception:
        pass
    return None, None, None, "missing"

def _read_cpu_pct(sample_s=0.35):
    def ticks():
    ...
    try:
        ...
        if dt <= 0:
            return None, "missing"
        return max(0.0, min(100.0, 100.0 * (1.0 - di / dt))), "measured"
    except Exception:
        return None, "missing"
...
def snapshot():
    ...
    return {"alias": "host", "host": os.uname().nodename, "fields": fields, "at": at, "source": "proc"}
...
def persist(snap):
    ensure_dirs()
    sample_path = SAMPLES / f"sys-{_local_stamp()}.json"
    _atomic_write(sample_path, snap)
    f = snap["fields"]
    _atomic_write(LAST / "host-last.json", snap)
    _atomic_write(CPU / "host-last.json", {"cpu_percent": f["cpu_percent"]["value"], "state": f["cpu_percent"]["state"], "at": snap["at"]})
    _atomic_write(LOAD / "host-last.json", {"load1": f["load1"]["value"], "load5": f["load5"]["value"], "load15": f["load15"]["value"], "state": f["load1"]["state"], "at": snap["at"]})
    _atomic_write(MEM / "host-last.json", {"mem_used_percent": f["mem_used_percent"]["value"], "mem_available_bytes": f["mem_available_bytes"]["value"], "mem_total_bytes": f["mem_total_bytes"]["value"], "state": f["mem_used_percent"]["state"], "at": snap["at"]})
    return sample_path
...
def summary_line(snap):
    ...
    return f"SYSTEM  cpu={v('cpu_percent', lambda x: f'{x:.0f}%')}  load={v('load1', lambda x: f'{x:.2f}')}  mem={v('mem_used_percent', lambda x: f'{x:.0f}%')}  src=proc"
