...
- Telegram `@carlymal_bot` via `skills/coms/telegram` council relay (**staged**); Grok room ≠ auto-wake from Telegram.
- Coding in Cursor unless operator says otherwise.
...
2. Grok Carly KB lives on box: `/workspace/carly-identity/kb/CARLY-KNOWLEDGEBASE.md` — OmniBook `carly-mal` / `energy-report` live trees still **Waiting** full verify from this lane.
3. Persona parity Telegram vs Grok room still pending (process locked: bak → seal → Bruce veto → desk sample).
...
| P1 | Persona parity draft for `carly-telegram` vs Grok Carly — bak → seal → Bruce veto | Local LLM must feel like this room |
...
- [ ] Draft `carly-telegram` persona parity diff vs Grok profile/KB; queue bak → seal → Bruce veto
